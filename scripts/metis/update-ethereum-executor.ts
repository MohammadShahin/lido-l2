...
async function main() {
  ...
  const [, mtsRunner] = ethMtsNetwork.getSigners(env.privateKey(), {
    forking: isForking,
  });
  ...
  console.log(`  · L2 tx runner: ${mtsRunner.address}`);
  console.log(`  · L2 tx runner ETH balance: ${await mtsRunner.getBalance()}`);
  ...
  const { callvalue, calldata } = await metis
  ...
  const createVoteTx = await oldEthExecutorLidoDAO.createVote(
    l1LDOHolder,
    "Update ethereumGovernanceExecutor on Metis Governance Bridge Executor",
    {
