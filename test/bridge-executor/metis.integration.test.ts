...
scenario("Metis :: Bridge Executor integration test", ctxFactory)
...
async function ctxFactory() {
  const networkName = env.network("TESTING_MTS_NETWORK", "mainnet");
  ...
  await metis.testing(networkName).stubL1CrossChainMessengerContract();
  ...
  const [, l2DeployScript] = await metis
  ...
  const mtsContracts = metis.contracts(networkName, { forking: true });

  const l1CrossDomainMessengerAliased = await testing.impersonate(
    testing.accounts.applyL1ToL2Alias(
      mtsContracts.L1CrossDomainMessenger.address
    ),
    l2Provider
  );

  const l2CrossDomainMessenger =
    await mtsContracts.L2CrossDomainMessenger.connect(
