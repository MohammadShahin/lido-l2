...
scenario("Metis :: Bridging via deposit/withdraw E2E test", ctxFactory)
  ...
  .step(
    "Bridge tokens from L1 to L2 via depositERC20ByChainId",
    async (ctx) => {
      const l2Gas = 5000000;
      depositTokensTxResponse =
        await ctx.l1ERC20TokenBridge.depositERC20ByChainId(
          ctx.chainIdTo,
          ctx.l1Token.address,
          ctx.l2Token.address,
          ctx.depositAmount,
  ...
  .step("Withdraw tokens from L2 via withdraw", async (ctx) => {
    const l1Gas = 5000000;
    withdrawTokensTxResponse = await ctx.l2ERC20TokenBridge.withdraw(
      ctx.l2Token.address,
