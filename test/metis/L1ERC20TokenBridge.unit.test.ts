...
import {
  ...
  MVMDiscountOracleStubMetis__factory,
  LibAddressManagerStubMetis__factory,
} from "../../typechain";
...
unit("Metis :: L1ERC20TokenBridge", ctxFactory)
  ...
  .test("addressmgr()", async (ctx) => {
    assert.equal(
      await ctx.l1TokenBridge.addressmgr(),
      ctx.stubs.libAddressManager.address
    );
  })
  ...
  .test("depositERC20()", async (ctx) => {
    ...
    await assert.emits(l1TokenBridge, tx, "ERC20ChainID", [chainId]);
...
async function ctxFactory() {
  ...
  const l1MessengerStub = await new CrossDomainMessengerStubMetis__factory(
    deployer
  ...
  // new contracts for metis
  const mvmDiscountOracleStub = await new MVMDiscountOracleStubMetis__factory(
    deployer
  ).deploy();

  const libAddressManagerStub = await new LibAddressManagerStubMetis__factory(
    deployer
  ).deploy(mvmDiscountOracleStub.address);

  const l1TokenBridgeImpl = await new L1ERC20TokenBridgeMetis__factory(
    deployer
  ).deploy(
    l1MessengerStub.address,
    l2TokenBridgeEOA.address,
  ...
  return {
    ...
    stubs: {
      ...
      mvmDiscountOracle: mvmDiscountOracleStub,
      libAddressManager: libAddressManagerStub,
    },
    l1TokenBridge,
  };
}
