...
import { L1ERC20TokenBridgeMetis__factory } from "../../typechain";

const REVERT = env.bool("REVERT", true);

scenario("Metis :: Launch integration test", ctxFactory)
  .after(async (ctx) => {
    if (REVERT) {
...
async function ctxFactory() {
  ...
  const { l1Provider, l2Provider, l1ERC20TokenBridge } = await metis
    .testing(networkName)
  ...
  const l1ERC20TokenBridgeImpl = L1ERC20TokenBridgeMetis__factory.connect(
